...
contract Bridged {
	...
	function dispatched(address target, bytes memory call) payable public onlyRelayer returns(bool success, bytes memory response) {
		console.log("dispatched()");
		(success, response) = target.call{value: msg.value, gas: 100000}(call);
	}
	
	function queried(address target, bytes memory call) public view onlyRelayer returns(bool success, bytes memory response) {
		console.log("queried()");
		(success, response) = target.staticcall{gas: 100000}(call);
	}
	
	function relay(address target, bytes memory call, bool readonly, bytes4 callback) internal returns(uint nonce) {
		uint nonce = Relayer(relayer).relay(target, call, readonly, callback);	
	}
}
...
contract Relayer {
	...
	*/}
	...
	event Relayed(address caller, address target, bytes call, bool readonly, bytes4 callback, uint nonce);

	function relay(address target, bytes memory call, bool readonly, bytes4 callback) public returns(uint) {
		emit Relayed(msg.sender, target, call, readonly, callback, nonces[msg.sender]);
		nonces[msg.sender]++;
		return nonces[msg.sender];
	}

	event Dispatched(address indexed caller, bytes4 callback, bool success, bytes response, uint indexed nonce);

	function dispatch(address caller, address target, bytes memory call, bytes4 callback, uint nonce, uint16[] memory indices, bytes[] memory signatures) public {
		require(!dispatched[caller][nonce], "Already dispatched");
		address[] memory validators = getValidators();
		require(3 * indices.length > 2 * validators.length, "No supermajority");
		bytes memory message = abi.encode(caller, target, call, false, callback, nonce);
		...
		(bool success, bytes memory response) = Bridged(caller).dispatched(target, call);
		emit Dispatched(caller, callback, success, response, nonce);
		dispatched[caller][nonce] = true;
	}

	function query(address caller, address target, bytes memory call) public view returns(bool success, bytes memory response){
		require(caller.code.length > 0);
		(success, response) = Bridged(caller).queried(target, call);
	}

	event Resumed(address indexed caller, bytes call, bool success, bytes response, uint indexed nonce);

	function resume(address caller, bytes4 callback, bool success, bytes memory response, uint nonce, uint16[] memory indices, bytes[] memory signatures) payable public {
		require(!resumed[caller][nonce], "Already resumed");
		address[] memory validators = getValidators();
		require(3 * indices.length > 2 * validators.length, "No supermajority");
		bytes memory message = abi.encode(caller, callback, success, response, nonce);
		bytes32 hash = message.toEthSignedMessageHash();
		for (uint i=0; i<signatures.length; i++) {
			require(i == 0 || indices[i] > indices[i-1], "Wrong index");
			address signer = hash.recover(signatures[i]);
		...
		bytes memory call = abi.encodeWithSelector(callback, success, response, nonce);
		(bool success2, bytes memory response2) = caller.call{value: msg.value, gas: 100000}(call);

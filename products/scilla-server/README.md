...
This project includes the source files for a dockerized Scilla server with HTTP API and their kustomize deployment manifests.
...
1. Create a branch:
   ...
   ```sh
   git checkout -b users/<username>/add_scilla-server_to_staging_cluster
   ```
       ...
       staging:
         apps:
           scilla-server:
             ...
             path: products/scilla-server/cd/overlays/staging
             track: staging
             type: kustomize
     ```
...
3. Push the changes
   ...
   ```sh
   git add .
   git commit -m "Add scilla-server to staging cluster"
   git push origin users/<username>/add_scilla-server_to_staging_cluster
   ```
...
```sh
z app sync --cache-dir=.cache scilla-server
```
...
1. Create a branch:
   ...
   ```sh
   git checkout -b users/<username>/add_scilla-server_to_production_cluster
   ```
     ...
     clusters:
       production:
         apps:
           scilla-server:
             repo: https://github.com/Zilliqa/zilliqa-developer
             path: products/scilla-server/cd/overlays/production
             track: production
             type: kustomize
     ```
...
3. Push the changes
   ...
   ```sh
   git add .
   git commit -m "Add scilla-server to production cluster"
   git push origin users/<username>/add_scilla-server_to_production_cluster
   ```

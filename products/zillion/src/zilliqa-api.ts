...
export class ZilSdk {
   ...
   */
  static getSmartContractSubStateBatch = async (
    queryList: any[]
  ): Promise<any> => {
    let result;
  ...
  private static getActualSmartContractSubStateBatch = async (
    queryList: any[]
  ): Promise<any> => {
    try {
      const { blockchain, api_list } = store.getState().blockchain;
      const randomAPI = API_RANDOMIZER.fetchApi(
        blockchain as NetworkURL,
        api_list
      );
      const zilliqa = new Zilliqa(randomAPI);

      let response: any =
        await zilliqa.blockchain.getSmartContractSubStateBatch(queryList);

      if (
        !response.hasOwnProperty("batch_result") ||
        response.batch_result === undefined
      ) {
   ...
   */
  static getSmartContractSubState = async (
    impl: string,
    state: string,
    indices?: any
  ): Promise<any> => {
    let result;
    for (let attempt = 0; attempt < API_MAX_ATTEMPT; attempt++) {
      result = await ZilSdk.getActualSmartContractSubState(
        impl,
        state,
        indices
      );
  ...
  static isOperator = async (impl: string, address: string): Promise<any> => {
    ...
    const response = await ZilSdk.getSmartContractSubState(impl, "ssnlist", [
      address,
    ]);
  ...
  private static getActualBalance = async (address: string) => {
    try {
      ...
      if (
        ...
        response.result.balance === undefined
      ) {
        return "0";
      }
   ...
   */
  private static getActualSmartContractSubState = async (
    impl: string,
    state: string,
    indices?: any
  ) => {
    if (!impl) {
      console.error(
        "error: get contract sub state - no implementation contract found"
      );
      return OperationStatus.ERROR;
    }
    ...
    try {
      ...
      if (indices !== null) {
        response = await zilliqa.blockchain.getSmartContractSubState(
          impl,
          state,
          indices
        );

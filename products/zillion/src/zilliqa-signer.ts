...
export class ZilSigner {
  ...
  private static ledgerSign = async (txParams: any, ledgerIndex: number) => {
    ...
    try {
      ...
      try {
        const response = await zilliqa.provider.send(
        ...
        console.error(
          "something is wrong with broadcasting the transaction :%o",
          JSON.stringify(err)
        );
        return OperationStatus.ERROR;
      }
      ...
      console.error(
        "error ledger sign - something is wrong with signing the transaction: %o",
        JSON.stringify(err)
      );
      return OperationStatus.ERROR;
    } finally {
  ...
  private static zilPaySign = async (txParams: any) => {
    ...
    try {
      ...
      console.error(
        "error zilpay sign - something is wrong with broadcasting the transaction: %o",
        JSON.stringify(err)
      );
      return OperationStatus.ERROR;
    }
  ...
  private static sdkSign = async (txParams: any) => {
    ...
    try {
      ...
      console.error(
        "error sdk sign - something is wrong with broadcasting the transaction: ",
        JSON.stringify(err)
      );
      console.error(err);
      return OperationStatus.ERROR;

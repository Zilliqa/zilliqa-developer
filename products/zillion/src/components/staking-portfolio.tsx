...
function StakingPortfolio(props: any) {
  ...
  const data: DelegStakingPortfolioStats[] = useAppSelector(
    (state) => state.user.deleg_staking_portfolio_list
  );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "delegAmt",
        Cell: ({ row }: any) => (
          <span>{convertQaToCommaStr(row.original.delegAmt)}</span>
        ),
      },
      {
        Header: "rewards (ZIL)",
        ...
        Cell: ({ row }: any) => (
          <>
            <span
              data-for="rewards-tip"
              data-tip={convertQaToZilFull(row.original.rewards)}
            >
              {convertQaToCommaStr(row.original.rewards)}
            </span>
            <ReactTooltip
              id="rewards-tip"
              place="bottom"
  ...
  return (
    <>
      ...
      {loading === OperationStatus.COMPLETE && (
        <>
          {data.length === 0 && (
            <div className="d-block px-4 pb-3 text-left">
              <em>You have not deposited in any nodes yet.</em>
            </div>
          )}

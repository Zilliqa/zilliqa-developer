...
function Dashboard(props: any) {
  ...
  const isDelegStatsLoading = useAppSelector(
    (state) => state.user.is_deleg_stats_loading
  );
      ...
      case NetworkLabel.TESTNET:
        ...
        if (env === Environment.PROD) {
          ...
          Alert(
            ...
            "Please switch to Mainnet via ZilPay."
          );
        }
        break;
  ...
  useEffect(() => {
    if (userState.account_type === AccountType.ZILPAY) {
      ...
      if (zilPay) {
        ...
        const accountStreamChanged = zilPay.wallet
          .observableAccount()
          .subscribe((account: any) => {
            ...
            dispatch(
              UPDATE_ADDRESS({
                address_base16: account.base16,
                address_bech32: bech32,
              })
            );
          });

        const networkStreamChanged = zilPay.wallet
          .observableNetwork()
          .subscribe((net: string) => networkChanger(net));
  ...
  useEffect(() => {
    window.onbeforeunload = (e: any) => {
      e.preventDefault();
      e.returnValue =
        "The page auto retrieves data periodically. Please do not force refresh as you will lose your wallet connection.";
      setTimeout(() => {
        toast.dismiss();
      }, 8000);
      return Alert(
        "warn",
        "Warning",
        "The app auto retrieves data periodically. Please do not force refresh as you will lose your wallet connection."
      );
    };
  }, []);
  ...
  useEffect(() => {
    if (
      userState.selected_role === Role.OPERATOR &&
      userState.role === Role.DELEGATOR
    ) {
      Alert(
        "warn",
        "Warning",
        "You have been redirected to the delegator dashboard."
      );
    }
          ...
          <ul className="navbar-nav navbar-right">
            ...
            <li className="nav-item">
              <p className="px-1">
                {userState.address_bech32 ? (
                  <a
                    href={getAddressLink(userState.address_bech32, networkURL)}
            ...
            <li className="nav-item">
              <p className="px-1">
                {userState.balance
                  ? convertQaToCommaStr(`${userState.balance}`)
                  : "0.000"}{" "}
                ZIL
              </p>
            </li>
            ...
            {/* network */}
            <li className="nav-item">
              {networkURL === NetworkURL.TESTNET && (
                <p className="px-1">Testnet</p>
              )}
              {networkURL === NetworkURL.MAINNET && (
                <p className="px-1">Mainnet</p>
              )}
              {networkURL === NetworkURL.ISOLATED_SERVER && (
                <p className="px-1">Isolated Server</p>
              )}
            </li>
            ...
            <li className="nav-item">
              ...
              >
                <IconShuffle width="16" height="16" />
                {
                  ...
                  <span
                    badge={
                      userState.swap_deleg_modal_data.swapRecipientAddress
                        ? userState.swap_deleg_modal_data.requestorList.length +
                          1
                        : userState.swap_deleg_modal_data.requestorList.length
                    }
                  ></span>
                  ...
                  <div className="dropdown-notify-wrapper">
                    ...
                    {isTxnNotify && (
                      <IconCheckboxBlankCircle
                        width="10"
                        height="10"
                        className="dropdown-notify-icon"
                      />
                    )}
                  </div>
                </button>
                ...
                <div className="col-12">
                  <div className="d-flex justify-content-end">
                    <button
                      ...
                      onClick={pollData}
                      data-tip
                      data-for="refresh-tip"
                      disabled={
                        isRefreshDisabled ||
                        isDelegStatsLoading === OperationStatus.PENDING
                      }
                    >
                  ...
                  {userState.role === Role.DELEGATOR && (
                    <>
                      ...
                      <div
                        id="delegator-stats-details"
                        className="p-4 dashboard-card container-fluid"
                      >
                        <div className="row">
                          <div className="col">
                        ...
                        <div className="row">
                          ...
                          <div className="col-12 mt-2 px-4 text-center">
                            <div className="inner-section">
                              <h6 className="inner-section-heading px-4 pt-4 pb-3">
                                Deposits{" "}
                                <span data-tip data-for="deposit-question">
                                  <IconQuestionCircle
                                    width="16"
                                    height="16"
                          ...
                          >
                            <span>
                              This shows you the list of nodes which you have
                              staked your deposit in.
                            </span>
                          </ReactTooltip>
                        </div>
                      </div>

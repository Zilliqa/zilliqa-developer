...
function LedgerWallet(props: any) {
  ...
  const getLedgerAccounts = async (currentLedgerIndex: number) => {
    try {
      ...
      Alert(
        "error",
        "Unable to access ledger",
        "Have you unlock the PIN code?"
      );
    }
  };
  ...
  const unlockWallet = async (hwIndex: number) => {
    ...
    try {
      ...
      const base16Address = bech32ToChecksum(
        selectedLedgerAddress
      ).toLowerCase();
  ...
  return (
    <div className="wallet-access">
      ...
      <div
        ...
        tabIndex={-1}
        role="dialog"
        aria-labelledby="ledgerConnectModalLabel"
        aria-hidden="true"
      >
        <div
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
                  ...
                  {ledgerAccounts &&
                    ledgerAccounts.map((item) => (
                      <tr
                        ...
                        onClick={() => unlockWallet(item.hwIndex)}
                        key={item.hwIndex}
                      >
                        <td>#{item.hwIndex}</td>
              ...
              <button
                ...
                onClick={() => getLedgerAccounts(ledgerIndex + 1)}
              >
                Next Account
              </button>
              <button
                ...
                onClick={() => unlockWallet(defaultLedgerIndex)}
              >
                Use Default Index #{defaultLedgerIndex}
              </button>
            </div>
          </div>
        </div>
      ...
      <button
        ...
        className="btn btn-user-action mx-2"
        data-toggle="modal"
        data-target="#ledger-connect-modal"
        data-keyboard="false"
        data-backdrop="static"
        onClick={() => getLedgerAccounts(ledgerIndex)}
      >

...
function CompleteWithdrawalTable(props: any) {
  const data: PendingWithdrawStats[] = useAppSelector(
    (state) => state.user.pending_withdraw_list
  );
  const completeWithdrawAmt = useAppSelector(
    (state) => state.user.complete_withdrawal_amt
  );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "amount",
        Cell: ({ row }: any) => (
          <span>{convertQaToCommaStr(row.original.amount)}</span>
        ),
      },
    ],
    []
  ...
  return (
    <>
      {data.length !== 0 && (
        <div
          id="delegator-complete-withdraw-details"
          className="col-12 mt-2 px-1 py-3"
        >
          <div id="complete-withdraw-accordion">
            <div className="card text-center">
              <h6 className="inner-section-heading px-4 pt-4">
              ...
              <div className="text-center">
                <Spinner
                  ...
                  area={PromiseArea.PROMISE_GET_DELEG_STATS}
                />
              </div>
              <div
                className="card-header d-flex justify-content-between"
                id="complete-withdraw-accordion-header"
              >
                <div>
                  <span>
                    <em>
                      ...
                      <strong>
                        {convertQaToCommaStr(completeWithdrawAmt)}
                      </strong>{" "}
                      ZIL
                    </em>
                  </span>
              ...
              <div
                id="complete-withdraw-details"
                className="collapse"
                aria-labelledby="complete-withdraw-accordion-header"
                data-parent="#complete-withdraw-accordion"
              >
                <div className="card-body">
                  {data.length === 0 && (
                    <em>You have no ZILs ready for withdrawal yet.</em>
                  )}
          ...
          >
            <span>
              When you initiate a stake withdrawal, the amount is not withdrawn
              immediately.
            </span>
            <br />
            <span>
              The amount is processed at a certain block number and only

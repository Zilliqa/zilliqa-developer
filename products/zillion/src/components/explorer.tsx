...
function Explorer(props: any) {
  ...
  const [stakedNodeList, setStakedNodeList] = useState(
    [] as DelegStakingPortfolioStats[]
  );

  const [pendingWithdrawalList, setPendingWithdrawlList] = useState(
  ...
  useEffect(() => {
    ...
    trackPromise(
      ZilSdk.getSmartContractSubState(impl, "deposit_amt_deleg", [wallet])
        .then(async (contractState) => {
          if (
            contractState === undefined ||
            contractState === null ||
            contractState === OperationStatus.ERROR
          ) {
            return null;
          }
          ...
          const ssnContractState = await ZilSdk.getSmartContractSubState(
            impl,
            "ssnlist"
          );
          ...
          for (const ssnAddress in depositDelegList) {
            ...
            // compute zil rewards
            const delegRewards = new BigNumber(
  ...
  useEffect(() => {
    ...
    trackPromise(
      ZilSdk.getSmartContractSubState(impl, "withdrawal_pending", [wallet])
        .then(async (contractState) => {
          if (
            contractState === undefined ||
            contractState === null ||
          ...
          const blkNumPendingWithdrawal =
            contractState["withdrawal_pending"][wallet];

          // get min bnum req
          const blkNumReqState = await ZilSdk.getSmartContractSubState(
            impl,
            "bnum_req"
          );
          ...
          const currBlkNum =
            isRespOk(numTxBlk) === true
              ? new BigNumber(numTxBlk!).minus(1)
              : new BigNumber(0);
    ...
    if (darkMode.value === true) {
      return (
        <img
          ...
          alt="zillion"
          width="480px"
          className="mt-2 mb-4 zillion-logo"
        />
      );
    } else {
      return (
        <img
          src={ZillionLightLogo}
          alt="zillion"
          width="480px"
          className="mt-2 mb-4 zillion-logo"
        />
          ...
          <div className="cover-content col-12 text-center">
            ...
            >
              <div>
                <button
                  type="button"
                  className="btn btn-theme shadow-none mr-3"
                  onClick={toggleTheme}
                >
                  {darkMode.value === true ? (
            ...
            <div className="wallet-access">
              ...
              <div className="d-flex justify-content-center h-100">
                <div className="explorer-search mb-4">
                  <input
                    ...
                    value={explorerSearchAddress}
                    onKeyDown={handleKeyPress}
                    onChange={handleExplorerSearchAddress}
                    placeholder="Enter wallet address to check rewards"
                    maxLength={42}
                  />
                  <button
                    type="button"
                    className="btn explorer-search-icon shadow-none"
                    onClick={() => explorerCheckRewards()}
                  >
              ...
              <h6 className="explorer-wallet mt-4">
                {walletBase16Address && address}
              </h6>
            </div>
            ...
            {walletBase16Address && (
              <>
                ...
                <div
                  ...
                  className="p-4 dashboard-card container"
                >
                  <div className="row">
                    ...
                    <div className="col-12 text-center">
                      <Spinner
                        class="spinner-border dashboard-spinner mb-4"
                        area={PromiseArea.PROMISE_GET_EXPLORER_STATS}
                      />
                      <div className="row px-2 pb-3 align-items-center justify-content-center">
                        <div className="d-block deleg-stats-card">
                          <h3>Total Deposits</h3>
                          <span>
                            {convertQaToCommaStr(delegStats.totalDeposits)}
                          </span>
                        </div>
                        <div className="d-block deleg-stats-card">
                          <h3>Unclaimed ZIL Rewards</h3>
                          <span>
                            {convertQaToCommaStr(delegStats.zilRewards)}
                          </span>
                        </div>
                        <div className="d-block deleg-stats-card"></div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {walletBase16Address && (
              <>
                <div
                  ...
                  className="p-4 dashboard-card mb-4 container"
                >
                  <div className="row">
                    <div className="col">
                      <h5 className="card-title mb-4 text-left">
                        Nodes Staked
                      </h5>
                    </div>
            ...
            <button
              type="button"
              className="btn btn-user-action-cancel mt-2 mx-2"

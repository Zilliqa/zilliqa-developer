...
function DelegateStakeModal(props: any) {
  ...
  const delegateStake = async () => {
    ...
    if (!delegAmt) {
      Alert(
        ...
        "Stake amount shoud not be empty."
      );
      return null;
    } else {
      try {
        delegAmtQa = convertZilToQa(delegAmt);
        const isLessThanMinDeleg = new BigNumber(delegAmtQa).isLessThan(
          minDelegStake
        );
        ...
        if (isLessThanMinDeleg) {
          Alert(
            "error",
            "Invalid Stake Amount",
            "Minimum stake amount is " + minDelegStakeDisplay + " ZIL."
          );
          return null;
        }
        ...
        const combinedFeesZil = units.fromQa(
          new BN(combinedFees.toString()),
          units.Units.Zil
        );
        ...
        if (!isBalanceSufficient) {
          Alert(
            ...
            "Your wallet balance is insufficient to pay for the staked amount and gas fees combined. Total amount required is " +
              combinedFeesZil +
              " ZIL."
          );
          Alert(
            ...
            "Current gas fee is around " +
              units.fromQa(new BN(gasFeesQa), units.Units.Zil) +
              " ZIL."
          );
          return null;
        }
  ...
  const setDefaultStakeAmt = useCallback(() => {
    if (minDelegStake) {
      setDelegAmt(
        units.fromQa(new BN(minDelegStake), units.Units.Zil).toString()
      );
    } else {
      setDelegAmt("0");
    }
  ...
  return (
    <div
      id="delegate-stake-modal"
      className="modal fade"
      tabIndex={-1}
      role="dialog"
      aria-labelledby="delegateStakeModalLabel"
      aria-hidden="true"
    >
      <div
          ...
          {isPending ? (
            ...
            <>
              <div className="modal-header">
                <h5 className="modal-title" id="delegateStakeModalLabel">
                  Stake
                </h5>
                <button
                  type="button"
              ...
              <div className="modal-body">
                <div className="row node-details-wrapper mb-4">
                  ...
                  <div className="col node-details-panel">
                    ...
                    <span>
                      {convertToProperCommRate(stakeModalData.commRate).toFixed(
                        2
                      )}
                      %
                    </span>
                ...
                <p>
                  <small>
                    ...
                    <strong>{convertQaToCommaStr(balance.toString())}</strong>{" "}
                    ZIL
                  </small>
                </p>
                  ...
                  <ul>
                    <li>
                      <small>
                        ...
                        <strong>{minDelegStakeDisplay}</strong> ZIL.
                      </small>
                    </li>
                    <li>
                      <small>
                        Please ensure you have at least <strong>100 ZIL</strong>{" "}
                        after staking to pay for gas fees for future
                        transactions such as withdrawal.
                      </small>
                    </li>
                  </ul>
                </div>

      ...
      <tbody {...getTableBodyProps()}>
        {rows.map((row, i) => {
          ...
          return (
            <tr
              {...row.getRowProps()}
              onClick={() => handleNodeSelect(row.original)}
            >
...
function ReDelegateStakeModal(props: any) {
  ...
  const userBase16Address = useAppSelector(
    (state) => state.user.address_base16
  );
  ...
  const minDelegStake = useAppSelector(
    (state) => state.staking.min_deleg_stake
  );
  const nodeSelectorOptions = useAppSelector(
    (state) => state.staking.ssn_dropdown_list
  );
  const stakeModalData: StakeModalData = useAppSelector(
    (state) => state.user.stake_modal_data
  );

  const minDelegStakeDisplay = units.fromQa(
    new BN(minDelegStake),
    units.Units.Zil
  );
  ...
  const hasRewardToWithdraw = async () => {
    ...
    const last_reward_cycle_json = await ZilSdk.getSmartContractSubState(
      impl,
      "lastrewardcycle"
    );
    const last_buf_deposit_cycle_deleg_json =
    ...
    // compute rewards
    const delegRewards = new BN(
      await computeDelegRewards(impl, ssnChecksumAddress, userBase16Address)
    );

    if (delegRewards.gt(new BN(0))) {
      Alert(
        "info",
        "Unwithdrawn Rewards Found",
        "Please withdraw the rewards before transferring."
      );
      return true;
    }
    ...
    if (
      last_buf_deposit_cycle_deleg_json.last_buf_deposit_cycle_deleg[
        userBase16Address
      ].hasOwnProperty(ssnChecksumAddress)
    ) {
      const lastDepositCycleDeleg = parseInt(
        last_buf_deposit_cycle_deleg_json.last_buf_deposit_cycle_deleg[
          userBase16Address
        ][ssnChecksumAddress]
      );
      ...
      if (lastRewardCycle <= lastDepositCycleDeleg) {
        Alert(
          "info",
          "Buffered Deposits Found",
          "Please wait for the next cycle before transferring."
        );
        return true;
      }
    }
  ...
  const redeleg = async () => {
    ...
    if (!delegAmt) {
      ...
      try {
        ...
        Alert(
          "error",
          "Invalid Transfer Amount",
          "Please check your transfer amount again."
        );
        return null;
      }
    }
    ...
    if (new BN(delegAmtQa).gt(new BN(currentAmtQa))) {
      Alert(
        ...
        "Invalid Transfer Amount",
        "You only have " +
          convertQaToCommaStr(currentAmtQa) +
          " ZIL to transfer."
      );
      setIsPending("");
      return null;
  ...
  return (
    <div
      id="redeleg-stake-modal"
      className="modal fade"
      tabIndex={-1}
      role="dialog"
      aria-labelledby="redelegModalLabel"
      aria-hidden="true"
    >
      <div
        className="contract-calls-modal modal-dialog modal-dialog-centered modal-lg"
        role="document"
      >
          ...
          {isPending ? (
            ...
            <>
              <div className="modal-header">
                <h5 className="modal-title" id="withdrawRewardModalLabel">
                  Transfer Stake
                </h5>
                <button
                  type="button"
              ...
              <div className="modal-body">
                  ...
                  <>
                    ...
                    <div className="row node-details-wrapper mb-4">
                      ...
                      <div className="col node-details-panel">
                        ...
                        <span>
                          {convertQaToCommaStr(stakeModalData.delegAmt)} ZIL
                        </span>
                      </div>
                    </div>
                    ...
                    {!toSsn && (
                      <button
                        type="button"
                        className="mb-4 btn btn-contract btn-block shadow-none"
                        onClick={() => toggleNodeSelector()}
                      >
                        Select a node
                      </button>
                    )}
                      ...
                      <>
                        <div className="row node-details-wrapper mb-4">
                          <div className="col node-details-panel">
                            ...
                            <button
                              type="button"
                              className="btn btn-change-node shadow-none"
                              onClick={() => toggleNodeSelector()}
                            >
                              Change
                            </button>
                        ...
                        <div className="modal-label mb-2">
                          <strong>Enter transfer amount</strong>
                        </div>
                        <div className="input-group mb-4">
                          <input
                            ...
                            value={delegAmt}
                            onChange={handleDelegAmt}
                          />
                          <div className="input-group-append">
                            <span className="input-group-text pl-4 pr-3">
                              ZIL
                            </span>
                          </div>
                        </div>
                    ...
                    <div className="d-flex">
                      <button
                        type="button"
                        className="btn btn-user-action mt-2 mx-auto shadow-none"
                        onClick={redeleg}
                      >
                        Transfer Stake
                      </button>
                ...
                {showNodeSelector && (
                  <>
                    <h2 className="node-details-subheading mb-2">
                      Select a node to transfer to
                    </h2>
                    ...
                    <div className="d-flex">
                      <button
                        type="button"
                        className="btn btn-user-action-cancel mt-4 mx-auto shadow-none"
                        onClick={() => toggleNodeSelector()}
                      >
                        Back
                      </button>

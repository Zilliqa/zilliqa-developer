...
import {
  bech32ToChecksum,
  computeGasFees,
  convertBase16ToBech32,
  getTruncatedAddress,
  getZillionExplorerLink,
  isDigits,
  isRespOk,
  showWalletsPrompt,
  validateBalance,
...
function SwapDelegModal(props: any) {
  ...
  const swapDelegModalData: SwapDelegModalData = useAppSelector(
    (state) => state.user.swap_deleg_modal_data
  );
  ...
  const [showConfirmSendRequestBox, setShowConfirmSendRequestBox] =
    useState(false);
  ...
  const validateAddress = (address: string) => {
    if (
      !address ||
      address === "" ||
      (!validation.isAddress(address) && !validation.isBech32(address))
    ) {
      return false;
    }
    return true;
    ...
    if (!address || address === null) {
      ...
      if (isSsnAddress(address)) {
        Alert(
          "error",
          "Invalid Address",
          `Do not enter a node address. If you want to transfer your stake from one node to another node, go to "Staking Portfolio" > "Manage" > "Transfer Stake" `
        );
        address = "";
      }
    }
  ...
  const toggleConfirmSendRequestBox = () => {
    ...
    if (!validateAddress(targetRecipientAddr)) {
      Alert(
        "error",
        "Invalid Address",
        "Wallet address should be bech32 or checksum format."
      );
      return null;
    }
  ...
  const sendTxn = async (txnType: TransactionType, txParams: any) => {
    if ((await validateBalance(wallet)) === false) {
      ...
      Alert(
        "error",
        "Gas Fee Estimation",
        "Current gas fee is around " +
          units.fromQa(gasFees, units.Units.Zil) +
          " ZIL."
      );
      setIsPending("");
      return null;
    ...
    trackPromise(
      ZilSigner.sign(accountType as AccountType, txParams, ledgerIndex)
        .then((result) => {
  ...
  const confirmDelegSwap = async (requestorAddr: string) => {
    ...
    const requestorHasBuffOrRewards = await hasBufferedOrRewards(
      "ConfirmSwap",
      requestorAddr
    );
    ...
    const newDelegHasBuffOrRewards = await hasBufferedOrRewards(
      "ConfirmSwap",
      userAddress
    );
  ...
  const isValidSwap = (swapAddr: string) => {
    ...
    if (userAddress === swapAddr) {
      Alert(
        "error",
        "Invalid New Owner",
        "Please enter another wallet address other than the connected wallet."
      );
      return false;
    }
    ...
    if (swapDelegModalData.requestorList.includes(byStr20SwapAddr)) {
      let msg = `There is an existing request from ${getTruncatedAddress(
        swapAddr
      )}. Please accept or reject the incoming request first.`;
  ...
  const hasBufferedOrRewards = async (
    ...
    let targetName =
      address === userAddress
        ? "Your wallet"
        : invokerMethod === "RequestSwap"
        ? "The recipient"
        : "The requestor";

    const lrc = await ZilSdk.getSmartContractSubState(impl, "lastrewardcycle");
    const lbdc = await ZilSdk.getSmartContractSubState(
      impl,
      "last_buf_deposit_cycle_deleg",
      [wallet]
    );
    const deposit_amt_deleg_map = await ZilSdk.getSmartContractSubState(
      impl,
      "deposit_amt_deleg",
      [wallet]
    );
    const buff_deposit_deleg_map = await ZilSdk.getSmartContractSubState(
      impl,
      "buff_deposit_deleg",
      [wallet]
    );
    ...
    for (let ssnAddress in ssnlist) {
      ...
      const rewards = new BN(
        await computeDelegRewards(impl, ssnAddress, wallet)
      );
      ...
      if (
        lbdc["last_buf_deposit_cycle_deleg"][wallet].hasOwnProperty(ssnAddress)
      ) {
        const ldcd = parseInt(
          lbdc["last_buf_deposit_cycle_deleg"][wallet][ssnAddress]
        );
        if (lrc_o <= ldcd) {
      ...
      if (lrc_o_minus >= 0) {
        if (
          isRespOk(buff_deposit_deleg_map) &&
          buff_deposit_deleg_map["buff_deposit_deleg"][wallet].hasOwnProperty(
            ssnAddress
          )
        ) {
          if (
            buff_deposit_deleg_map["buff_deposit_deleg"][wallet][
              ssnAddress
            ].hasOwnProperty(lrc_o_minus)
          ) {
            let msg = `${targetName} ${displayAddr} has buffered deposits. Please wait for the next cycle before continuing.`;
            Alert("info", "Buffered Deposits Found", msg);
            return true;
  ...
  const hasPendingWithdrawal = async (address: string) => {
    ...
    const pending_withdrawal_map = await ZilSdk.getSmartContractSubState(
      impl,
      "withdrawal_pending",
      [wallet]
    );
  ...
  const requestDelegSwap = async () => {
    ...
    if (!userHasStaked && !userHasPendingWithdrawal) {
      ...
      Alert(
        "info",
        "User Has No Stake",
        `You have not staked with any operators.`
      );
      return null;
    }

    const userHasBuffOrRewards = await hasBufferedOrRewards(
      "RequestSwap",
      userAddress
    );
  ...
  const onBlurGasPrice = () => {
    if (
      gasPrice === "" ||
      new BigNumber(gasPrice).lt(new BigNumber(defaultGasPrice))
    ) {
      setGasPrice(defaultGasPrice);
      Alert(
        "Info",
        "Minimum Gas Price Required",
        "Gas price should not be lowered than default blockchain requirement."
      );
    }
  };
  ...
  return (
    <div
      ...
      tabIndex={-1}
      role="dialog"
      aria-labelledby="swapDelegModalLabel"
      aria-hidden="true"
    >
      <div
        className="contract-calls-modal modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg"
        role="document"
      >
          ...
          {isPending ? (
            ...
            <>
              <div className="modal-header">
                {tutorialStep === 0 ? (
                  <h5 className="modal-title" id="swap-deleg-modal">
                    What is this?
                  </h5>
                ) : tutorialStep === 1 ? (
                  <h5 className="modal-title" id="swap-deleg-modal">
                    How do I transfer my stakes to another wallet?
                  </h5>
                ) : tutorialStep === 2 ? (
                  <h5 className="modal-title" id="swap-deleg-modal">
                    How do I modify the recipient or revoke the request?
                  </h5>
                ) : tutorialStep === 3 ? (
                  <h5 className="modal-title" id="swap-deleg-modal">
                    I received an incoming transfer request. What should I do
                    with it?
                  </h5>
                ) : tutorialStep === 4 ? (
                  <h5 className="modal-title" id="swap-deleg-modal">
                    I accepted a request. How do I know if it is successful?
                  </h5>
                ) : tutorialStep === 5 ? (
                  <h5 className="modal-title" id="swap-deleg-modal">
                    Final Notes
              ...
              <div className="modal-body">
                {tutorialStep === 0 ? (
                  <div className="div-fade text-center">
                    <IconWalletTransferLong
                      width="400"
                      className="mb-4 icon-fill"
                    />
                    <p>
                      You can now transfer your stakes from one wallet to
                      another!
                    </p>
                    <button
                      type="button"
                      className="btn btn-user-action mx-auto shadow-none"
                  ...
                  <div>
                    <p>
                      Under the "Change Stake Ownership" tab, enter the
                    ...
                    <div className="d-flex mb-4">
                      <img
                        ...
                        src={SwapImg}
                        alt="change_ownership"
                      />
                    </div>
                    <p>
                      Wait for the transaction to be processed on the
                      blockchain.
                    </p>
                    <div className="d-flex mt-4">
                      <div className="mx-auto">
                        <button
                          type="button"
                          className="btn btn-user-action-cancel mx-2 shadow-none"
                          onClick={() => decreaseTutorialStep()}
                        >
                          Prev
                        </button>
                        <button
                  ...
                  <div>
                    <p>
                      After setting a recipient, should you change your mind,
                      you may edit the recipient to another address or revoke
                    ...
                    <div className="d-flex mb-4">
                      <img
                        className="mx-auto img-fluid"
                        src={SwapImg1}
                        alt="edit_revoke_request"
                      />
                    </div>
                    <p>
                      This can only be done if the recipient has not accepted
                      your request.
                    </p>
                    <div className="d-flex mt-4">
                      <div className="mx-auto">
                  ...
                  <div>
                    <p>
                      Under the "Incoming Requests", if you are the{" "}
                      <strong>recipient</strong>, you will see the list of
                      wallet addresses / users who wish to transfer their stakes
                      to you.
                    </p>
                    <div className="d-flex mb-4">
                      <img
                        className="mx-auto img-fluid"
                        src={SwapImg2}
                        alt="incoming_requests"
                      />
                    </div>
                    <p>
                      If you <strong>accept</strong> the requests, all the
                      stakes would be transferred from the requestors' wallet to
                      you.
                    </p>
                    <p>
                      If you <strong>reject</strong> the requests, nothing would
                      happen and the requestor has to send a new request if he
                      or she wishes to transfer the stakes to you.
                    </p>
                    <p>
                      Please wait for the transaction to be processed completely
                      on the blockchain before accepting or rejecting another
                      request.
                    </p>
                    <div className="d-flex mt-4">
                      <div className="mx-auto">
                  ...
                  <div>
                    <p>
                      If you are the <strong>recipient</strong>, you should see
                      a transaction ID after accepting the request as shown
                      below.
                    </p>
                    <div className="d-flex mb-4">
                      <img
                    ...
                    <p>
                      Click on the transaction ID and it would bring you to
                      ViewBlock.
                    </p>
                    <p>
                      Refresh the ViewBlock page once every few minutes; the
                      transaction should be successful in less than 10 minutes.
                    </p>
                    <p>
                      Head back to <strong>Zillion Dashboard</strong> and hit
                    ...
                    <p>
                      You should observe the new amounts under{" "}
                      <strong>My Staking Portfolio</strong>.
                    </p>
                    <div className="d-flex mb-4">
                      <img
                        className="mx-auto img-fluid"
                        src={SwapImg5}
                        alt="dashboard_staking_portfolio"
                      />
                    </div>
                    <div className="d-flex mt-4">
                      <div className="mx-auto">
                  ...
                  <div>
                    <ul>
                      <li className="mb-3">
                        <strong>
                          Once the recipient has accepted a request, Zilliqa is
                          not able to undo the transfer process.
                        </strong>
                      </li>
                      <li className="mb-3">
                        Beware of scams! Do not send a transfer request if you
                        are unsure of who the recipient is!
                      </li>
                      <li className="mb-3">
                        Do not enter any node operator's address.
                      </li>
                      <li className="mb-3">
                        If you are a recipient, once you accept the transfer
                        request, all the stakes would be transferred to your
                        wallet. If the requestor has staked on the same node
                        operator as you, the amount would be tabulated together.
                      </li>
                      <li className="mb-3">
                        If you are a requestor, please check the recipient
                        address carefully before sending the request. The
                        transfer request is <strong>irreversible</strong> once
                        the recipient has accepted.
                      </li>
                      <li>
                        Ensure that you have no buffered deposits or unwithdrawn
                        rewards before sending a transfer request or accepting a
                        transfer. This is to facilitate the stakes tabulation
                        when the recipient chooses to accept the request.
                      </li>
                    </ul>
                    <div className="d-flex mt-4">
                      <div className="mx-auto">
            ...
            <div className="modal-body animate__animated animate__fadeIn">
              ...
              <div className="mb-4">
                ...
                <ul>
                  <li>
                    <small>
                      By clicking on <em>'Yes'</em>, you are sending a request
                      to transfer all your existing stakes to this address.
                    </small>
                  </li>
                  <li>
                    <small>
                      Beware of scams! Ensure that you know who the recipient is
                      before sending the request.
                    </small>
                  </li>
                  <li>
                    <small>
                      Process is <strong>irreversible</strong> once the
              ...
              <div className="d-flex mt-4">
                <div className="mx-auto">
                  <button
                    type="button"
                    className="btn btn-user-action-cancel mx-2 shadow-none"
                    onClick={() => setShowConfirmSendRequestBox(false)}
                  >
                    Cancel
                  </button>
                  <button
            ...
            <div className="modal-body animate__animated animate__fadeIn">
              ...
              <p>
                Are you sure you want to revoke the existing transfer ownership
                request?
              </p>
              <div className="row node-details-wrapper mb-4">
                <div className="col node-details-panel">
              ...
              <div className="d-flex mt-4">
                <div className="mx-auto">
                  <button
                    type="button"
                    className="btn btn-user-action-cancel mx-2 shadow-none"
                    onClick={() => setShowConfirmRevokeBox(false)}
                  >
                    Cancel
                  </button>
                  <button
            ...
            <div className="modal-body animate__animated animate__fadeIn">
              ...
              <div className="row node-details-wrapper mb-4">
                <div className="col node-details-panel">
                  ...
                  <a
                    href={getZillionExplorerLink(
                      convertBase16ToBech32(selectedDelegAddr)
                    )}
                    target="_blank"
                    rel="noopener noreferrer"
                    data-tip
                    data-for="explorer-tip"
                  >
                    <IconEditBox
                      width="16"
                      height="16"
                      className="zillion-explorer-link"
                    />
                  </a>
                  <ReactTooltip
                    id="explorer-tip"
              ...
              <div className="mb-4">
                ...
                <ul>
                  <li>
                    <small>
                      By clicking on <em>'Yes'</em>, you are rejecting the
                      transfer request and would not receive the stakes from the
                      requestor's wallet.
                    </small>
                  </li>
                </ul>
              ...
              <div className="d-flex mt-4">
                <div className="mx-auto">
                  <button
                    type="button"
                    className="btn btn-user-action-cancel mx-2 shadow-none"
                    onClick={() => setShowConfirmRejectBox(false)}
                  >
                    Cancel
                  </button>
                  <button
                    type="button"
                    className="btn btn-user-action mx-2 shadow-none"
                    onClick={() => rejectDelegSwap(selectedDelegAddr)}
                  >
                    Yes
                  </button>
                </div>
            ...
            <div className="modal-body animate__animated animate__fadeIn">
              ...
              <p>
                ...
                all the stakes from this wallet?
              </p>
              ...
              <div className="mb-4">
                ...
                <ul>
                  <li>
                    <small>
                      By clicking on <em>'Yes'</em>, all the stakes are
                      transferred from the requestor's wallet to your wallet.
                    </small>
                  </li>
                  <li>
                    <small>
                      This transfer process is <strong>irreversible.</strong>
                    </small>
                  </li>
                </ul>
              </div>
              ...
              <div className="d-flex mt-4">
                <div className="mx-auto">
                  <button
                    type="button"
                    className="btn btn-user-action-cancel mx-2 shadow-none"
                    onClick={() => setShowConfirmSwapBox(false)}
                  >
                    Cancel
                  </button>
                  <button
                    type="button"
                    className="btn btn-user-action mx-2 shadow-none"
                    onClick={() => confirmDelegSwap(selectedDelegAddr)}
                  >
                    Yes
                  </button>
                </div>
            ...
            <>
              <div className="d-flex">
                <button
                  type="button"
                  className="btn btn-notify-dropdown btn-theme ml-auto mt-1 mr-2"
                  aria-label="Help"
                  data-tip
                  data-for="tutorial-tip"
                ...
                <button
                  type="button"
                  className="close btn shadow-none mx-2 mt-2"
                  data-dismiss="modal"
                  aria-label="Close"
                  onClick={handleClose}
                >
              ...
              >
                <TabList>
                  ...
                  <Tab>
                    ...
                    {swapDelegModalData.requestorList.length > 0 ? (
                      <span>({swapDelegModalData.requestorList.length})</span>
                    ) : null}
                  </Tab>
                </TabList>
                ...
                <TabPanel>
                  <div className="modal-body">
                    <p className="mb-4">
                      Transfer <strong>ALL</strong> your existing stakes from
                      one wallet to another by entering a new owner address.
                    </p>
                    {swapDelegModalData.swapRecipientAddress ? (
                      <div className="row node-details-wrapper mb-4">
                        <div className="col node-details-panel">
                      ...
                      isEdit || !swapDelegModalData.swapRecipientAddress ? (
                        <div className="edit-swap-addr">
                          <div className="modal-label mb-2">
                            ...
                            <br />
                            (in bech32 format e.g. zil1xxxxxxxxxxxxx)
                          </div>
                    ...
                    <div className="edit-swap-notes">
                      ...
                      <ul>
                        <li>
                          <small>
                            Beware of scams! Ensure that you know who the
                            recipient is before sending the request.
                          </small>
                        </li>
                        <li>
                          <small>
                        ...
                        <div className="mx-auto">
                          {isEdit ? (
                            ...
                            <button
                              type="button"
                              className="btn btn-user-action mx-2 shadow-none"
                              onClick={() => toggleConfirmSendRequestBox()}
                            >
                              Send Request
                            </button>
                          ) : (
                  ...
                  {swapDelegModalData.requestorList.length === 0 ? (
                    ...
                    <>
                      <ul className="p-3 list-unstyled swap-deleg-requestor-list">
                        {swapDelegModalData.requestorList.map(
                          (requestorAddr: string, index: number) => (
                            <li key={index}>
                              <div className="flex mb-2">
                                <span>
                                  {convertBase16ToBech32(requestorAddr)}
                                </span>
                                <a
                                  href={getZillionExplorerLink(
                                    convertBase16ToBech32(requestorAddr)
                                ...
                                <div className="float-right btn-contract-group">
                                  <button
                                    type="button"
                                    className="btn btn-contract-small shadow-none mx-2"
                                    onClick={() =>
                                      toggleConfirmSwapBox(requestorAddr)
                                    }
                                  >
                                    Accept
                                  </button>
                                  <button
                                    type="button"
                                    className="btn btn-contract-small-cancel shadow-none mx-2"
                                    onClick={() =>
                                      toggleRejectSwapBox(requestorAddr)
                                    }
                                  >
                                    Reject
                                  </button>

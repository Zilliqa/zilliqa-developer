...
function UpdateCommRateModal(props: any) {
  ...
  const commRate = useAppSelector(
    (state) => state.user.operator_stats.commRate
  );
  ...
  const updateCommRate = async () => {
    if (!newRate || !newRate.match(/\d/)) {
      Alert(
        ...
        "Please enter an appropriate value, e.g. 10.5"
      );
      return null;
    }
    ...
    if (newRate.length > 9) {
      Alert(
        "error",
        "Invalid Commission Rate",
        "Commission rate should have a maximum of 7 decimals only."
      );
      return null;
    }
  ...
  return (
    <div
      id="update-comm-rate-modal"
      className="modal fade"
      tabIndex={-1}
      role="dialog"
      aria-labelledby="updateCommRateModalLabel"
      aria-hidden="true"
    >
      <div
        className="contract-calls-modal modal-dialog modal-dialog-centered"
        role="document"
      >
          ...
          {isPending ? (
            ...
            <>
              <div className="modal-header">
                <h5 className="modal-title" id="updateCommRateModalLabel">
              ...
              <div className="modal-body">
                <div className="row node-details-wrapper mb-4">
                  <div className="col node-details-panel">
                    ...
                    <span>
                      {commRate
                        ? convertToProperCommRate(commRate).toFixed(2)
                        : "0.00"}
                      &#37;
                    </span>
                  </div>

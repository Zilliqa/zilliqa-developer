...
import {
  ...
  convertQaToCommaStr,
  computeStakeAmtPercent,
  getTruncatedAddress,
  computeTotalStakeAmt,
} from "../util/utils";
              ...
              {headerGroup.headers.map((column, index) => (
                <th scope="col" {...column.getHeaderProps()}>
                  {column.render("tipText") === "" ? (
                    ...
                    <span
                      className="ssn-table-header-with-tip"
                      data-for="ssn-table-header-tip"
                      data-tip={column.render("tipText")}
                    >
                      {column.render("Header")}
                    </span>
                  )}
...
function SsnTable(props: any) {
  ...
  const loading: OperationStatus = useAppSelector(
    (state) => state.staking.is_ssn_stats_loading
  );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "apiUrl",
        Cell: ({ row }: any) => (
          <span className="ssn-table-api-url">{row.original.apiUrl}</span>
        ),
        tipText:
          "Service API running by operator. Can be used as alternatives for Zilliqa API endpoint",
      },
      {
        ...
        Cell: ({ row }: any) => (
          <>
            <span>
              {convertQaToCommaStr(row.original.stakeAmt)} (
              {computeStakeAmtPercent(
                row.original.stakeAmt,
                totalStakeAmt
              ).toFixed(2)}
              &#37;)
            </span>
      ...
      },
      {
        ...
        tipText:
          "Total staked amount deposited in this cycle being considered for rewards in the next cycle",
      },
      {
        Header: "Comm. Rate (%)",
        accessor: "commRate",
        ...
        tipText:
          "Percentage of incoming rewards that the operator takes as commission",
      },
      {
        Header: "Comm. Reward (ZIL)",
        accessor: "commReward",
        Cell: ({ row }: any) => (
          <span className="ssn-table-comm-reward">
      ...
      },
      {
        ...
        Cell: ({ row }: any) => (
          <>
            <div
              className={
                row.original.status === SsnStatus.ACTIVE
                  ? "px-2 py-1 rounded ssn-table-status-active"
                  : "px-2 py-1 rounded ssn-table-status-inactive"
              }
            >
        ...
        tipText:
          "Determines whether the operator has met the minnimum stake amount and therefore ready to participate in staking and receive rewards",
      },
      {
        Header: "Stake",
        accessor: "stake",
            ...
            <button
              ...
              onClick={() =>
                handleStake(
                  ...
                  row.original.commRate
                )
              }
              disabled={
                ContractState.IS_PAUSED.toString() === "true" ? true : false
              }
            >
              Stake
            </button>

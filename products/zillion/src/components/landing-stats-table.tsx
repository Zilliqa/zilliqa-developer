...
function LandingStatsTable(props: any) {
  const loading: OperationStatus = useAppSelector(
    (state) => state.staking.is_landing_stats_loading
  );
  const landingStats: LandingStats = useAppSelector(
    (state) => state.staking.landing_stats
  );
            ...
            {loading === OperationStatus.COMPLETE && (
              <>
                <div className="row pb-3 mx-auto justify-content-center">
                  ...
                  <div className="d-block landing-stats-card"></div>
                </div>
              </>
            )}
          </div>

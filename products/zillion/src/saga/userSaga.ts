...
function* queryAndUpdateRole() {
  ...
  try {
    ...
    const isOperator: boolean = yield call(
      ZilSdk.isOperator,
      impl,
      address_base16
    );
...
function* queryAndUpdateGzil() {
  ...
  try {
    ...
    if (gzil_address && gzil_address !== null) {
      ...
      const { gziladdr } = yield call(
        ZilSdk.getSmartContractSubState,
        impl,
        "gziladdr"
      );
    ...
    if (!isRespOk(response)) {
      ...
      yield put(
        UPDATE_GZIL_BALANCE({
          gzil_balance: (response as any)["balances"][address_base16],
        })
      );
    }
  } catch (e) {
...
function* populateStakingPortfolio() {
  ...
  try {
    ...
    const depositAmtDeleg =
      isRespOk(response) === true
        ? (response as any)["deposit_amt_deleg"][address_base16]
        : null;
    ...
    for (const ssnAddress in depositAmtDeleg) {
      ...
      // compute zil rewards
      const delegRewards: string = yield call(
        computeDelegRewards,
        impl,
        ssnAddress,
        address_base16
      ...
      const stakingStats: DelegStakingPortfolioStats = {
        ssnName:
          (ssn_list as SsnStats[]).find(
            (ssn) => ssn.address === ssnAddressBech32
          )?.name || "",
    ...
    yield put(
      UPDATE_DELEG_PORTFOLIO({ portfolio_list: staking_portfolio_list })
    );
  } catch (e) {
...
function* populateSwapRequests() {
  ...
  try {
    const { address_base16 } = yield select(getUserState);
    const { impl } = yield select(getBlockchain);
    const { deleg_swap_request } = yield call(
      ZilSdk.getSmartContractSubState,
      impl,
      "deleg_swap_request"
    );
    ...
    // reverse the mapping
    let reverseMap = Object.keys(deleg_swap_request).reduce(
      (newMap: any, k) => {
    ...
    console.warn(e);
    yield put(
      UPDATE_SWAP_DELEG_MODAL({ swap_deleg_modal: initialSwapDelegModalData })
    );
  }
}
...
function* populatePendingWithdrawal() {
  ...
  try {
    ...
    const pendingWithdrawal = (response as any)["withdrawal_pending"][
      address_base16
    ];
    ...
    yield put(
      UPDATE_PENDING_WITHDRAWAL_LIST({
        pending_withdraw_list: pendingWithdrawList,
      })
    );
    yield put(
      UPDATE_COMPLETE_WITHDRAWAL_AMT({
        complete_withdrawal_amt: `${totalWithdrawAmt}`,
      })
    );
  } catch (e) {
...
function* populateRewardBlkCountdown() {
  try {
    ...
    yield put(
      UPDATE_REWARD_BLK_COUNTDOWN({
        reward_blk_countdown: `${rewardBlkCountdown}`,
      })
    );
  } catch (e) {
...
function* populateOperatorStats() {
  ...
  try {
    ...
    // get number of deleg
    const delegQuery: Object = yield call(
      ZilSdk.getSmartContractSubState,
      impl,
      "ssn_deleg_amt",
      [address_base16]
    ...
    const delegNum =
      isRespOk(delegQuery) === true
        ? Object.keys((delegQuery as any)["ssn_deleg_amt"][address_base16])
            .length
        : 0;
...
function* watchPollUserData() {
  while (true) {
    ...
    yield race([call(pollUserSaga), take(POLL_USER_DATA_STOP)]);
  }
}

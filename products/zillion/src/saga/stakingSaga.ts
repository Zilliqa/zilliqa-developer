...
function* watchInitOnce() {
  try {
    ...
    const response: Object = yield call(
      ZilSdk.getSmartContractSubStateBatch,
      queries
    );
    ...
    const { last_withdraw_cycle_deleg } = yield call(
      ZilSdk.getSmartContractSubState,
      impl,
    ...
    if (isRespOk(last_withdraw_cycle_deleg)) {
      const { comm_for_ssn } = yield call(
        ZilSdk.getSmartContractSubState,
        impl,
        "comm_for_ssn"
      );
      const nodesNum = isRespOk(comm_for_ssn)
        ? Object.keys(comm_for_ssn).length.toString()
        : "0";
      ...
      if (isRespOk(totalCoinSupply)) {
        const totalCoinSupplyBN = new BigNumber(
          convertZilToQa(totalCoinSupply)
        );
        circulatingSupplyStake = new BigNumber(totalstakeamount)
          .dividedBy(totalCoinSupplyBN)
          .times(100)
          .toFixed(5);
      }
      ...
      const remainingGzil = remainGzil
        .dividedBy(maxGzilSupply)
        .times(100)
        .toFixed(2);
    ...
    yield put(
      UPDATE_TOTAL_STAKE_AMOUNT({ total_stake_amount: totalstakeamount })
    );
...
function* watchPollStakingData() {
  while (true) {
    ...
    yield race([call(pollStakingSaga), take(POLL_STAKING_DATA_STOP)]);
  }
}

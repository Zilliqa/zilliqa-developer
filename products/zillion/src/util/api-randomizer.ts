...
export class ApiRandomizer {
  ...
  public fetchApi(url: NetworkURL, apiList: string[]) {
    if (url === NetworkURL.MAINNET && this.mainnetApiList.length === 0) {
      this.mainnetApiList =
        apiList.length > 0 ? [...apiList] : ["https://api.zilliqa.com"];
      this.randMainnetIndexList = randomJS.shuffle(
        Array.from(Array(this.mainnetApiList.length).keys())
      );
    } else if (url === NetworkURL.TESTNET && this.testnetApiList.length === 0) {
      this.testnetApiList =
        apiList.length > 0 ? [...apiList] : ["https://dev-api.zilliqa.com"];
      this.randTestnetIndexList = randomJS.shuffle(
        Array.from(Array(this.testnetApiList.length).keys())
      );
    }

...
// returns a BigNumber
export const computeStakeAmtPercent = (
  inputStake: string,
  totalStake: string
) => {
...
export const getAddressLink = (address: string, networkURL: string) => {
  ...
  if (blockchain_explorer_config === Explorer.VIEWBLOCK) {
    if (networkURL === NetworkURL.MAINNET) {
      ...
      link =
        "https://viewblock.io/zilliqa/address/" + address + "?network=testnet";
    }
  } else {
    // devex
    link =
      "https://devex.zilliqa.com/address/" + address + "?network=" + networkURL;
  }
...
export const calculateBlockRewardCountdown = (
  blockNum: number,
...
export const isRespOk = (obj: any): boolean => {
  if (obj !== undefined && obj !== null && obj !== OperationStatus.ERROR) {
    return true;
  }
  return false;
};

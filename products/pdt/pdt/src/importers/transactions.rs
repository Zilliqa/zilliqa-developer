...
use pdtdb::{values, zqproj::Inserter};
...
pub struct TransactionMicroblockImporter {
    txn_inserter: Option<Inserter<values::BQTransaction>>,
    mb_inserter: Option<Inserter<values::BQMicroblock>>,
...
impl Importer for TransactionMicroblockImporter {
    ...
    async fn extract_start<P: ZilliqaDBProject + std::marker::Sync>(
        ...
        self.txn_inserter = Some(project.make_inserter::<values::BQTransaction>().await?);
        self.mb_inserter = Some(project.make_inserter::<values::BQMicroblock>().await?);
        self.range = None;
        Ok(())
    }
        ...
        for val in exporter.micro_blocks(range.end.try_into()?, Some(range.start.try_into()?))? {
            if let Ok((key, blk)) = val {
                ...
                if let Some(mb_inserter) = self.mb_inserter.as_mut() {
                    mb_inserter.insert_row(values::BQMicroblock::from_proto(
                        &blk.clone(),
                ...
                for (_hash, maybe_txn) in exporter.txns(&key, &blk)? {
                    ...
                    if let Some(txn) = maybe_txn {
                        if let Some(inserter) = self.txn_inserter.as_mut() {
                            inserter.insert_row(values::BQTransaction::from_proto(
                                &txn.clone(),
                                <i64>::try_from(key.epochnum)?,
                                offset_in_block,
                                <i64>::try_from(key.shardid)?,

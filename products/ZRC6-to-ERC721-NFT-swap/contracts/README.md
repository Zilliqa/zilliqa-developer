...
* **Parameters:**  
  1. scilla\_nft\_ids\_to\_swap (uint256\[\]): An array of ZRC-6 NFT token IDs to be burned and swapped.  
* **Execution Logic:**  
  1. **Authentication & Verification:**  
     * The contract must verify that the caller (msg.sender) is approved as an operator on the Scilla NFT contract for the specified NFTs. This approval must be set up beforehand using ZilPay.  
     * After verifying the approval, the contract must use the Zilliqa interoperability layer to confirm that the caller is authorized to burn the NFTs on the Scilla side. If the authorization check fails for any NFT, the transaction must revert.  

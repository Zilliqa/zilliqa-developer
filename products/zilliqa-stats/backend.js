...
const query = gql`
  query Transactions($page: Int, $perPage: Int) {
    txPagination(page: $page, perPage: $perPage, sort: TIMESTAMP_DESC) {
      pageInfo {
        currentPage
        perPage
      }
...
export const requestApollo = async (page) => {
  try {
    ...
    const data = await request("https://devex-apollo.zilliqa.com", query, {
      page,
      perPage: 20000,
    });
...
export const runBackend = async (statsService) => {
  ...
  await requestViewblockTokens(
    "zETH",
    "zil19j33tapjje2xzng7svslnsjjjgge930jx0w09v",
    "ethereum"
  );
  await sleep(2000);
  await requestViewblockTokens(
  ...
  await requestViewblockTokens(
    "zWBTC",
    "zil1wha8mzaxhm22dpm5cav2tepuldnr8kwkvmqtjq",
    "wrapped-bitcoin"
  );
  await sleep(2000);
  await requestViewblockTokens(
  ...
  const filtered = transactions.filter(
    (i) => parseInt(i.timestamp / 1000) > now
  );

  const cumulative = fs.readJsonSync("cumulative.json");
  const cfiltered = transactions.filter(
    (i) => parseInt(i.timestamp / 1000) > cumulative.timestamp
  );

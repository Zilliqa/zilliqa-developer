...
class StatsService {
  ...
  syncData = async () => {
    ...
    this.txs = this.txsraw.transactions.map((item) => {
      return {
        ...item,
        date_formatted: dayjs(parseInt(item.timestamp / 1000)).format(
          "YYYY-MM-DD"
        ),
      };
    });
    this.vballraw = fs.readJsonSync("./viewblock-2Y.json");
    this.vball = this.vballraw.timeData.map((item) => {
      return {
        ...item,
        date_formatted: dayjs(item.timestamp).format("YYYY-MM-DD"),
      };
    });
    this.vb1wraw = fs.readJsonSync("./viewblock-1W.json");
    this.vb1w = this.vb1wraw.timeData.map((item) => {
      return {
        ...item,
        date_formatted: dayjs(item.timestamp).format("YYYY-MM-DD"),
      };
  ...
  getZilsBurnt = async () => {
    ...
    for (const [key, value] of Object.entries(grouped)) {
      ...
      details.push({
        time: key,
        value: units.fromQa(new BN(zils_burnt.toString()), units.Units.Zil),
      });
    }
  ...
  try {
    ...
    res.json({
      ...
      totalSupplyFormatted:
        Number(tokenraw.totalSupply) / Math.pow(10, tokenraw.decimals),
      totalValue:
        (Number(tokenraw.totalSupply) / Math.pow(10, tokenraw.decimals)) *
        tokenraw.coingeckoValue,
    });
  } catch (error) {
    res.json({
      error: true,
      message: `There has been an error trying to read ${req.params.symbol} token file.`,

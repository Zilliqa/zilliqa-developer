...
impl AstVisitor for NodeTypeNameIdentifier {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                NodeTypeNameIdentifier::ByteStringType(bs_type) => bs_type.node.visit(emitter),
                _ => Ok(TraversalResult::Continue),
            }
        } else {
            ret
...
impl AstVisitor for NodeImportedName {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                NodeImportedName::RegularImport(name) => name.node.visit(emitter),
                NodeImportedName::AliasedImport(name, alias) => match name.node.visit(emitter) {
                    Err(msg) => Err(msg),
                    _ => alias.node.visit(emitter),
                },
            }
        } else {
            ret
...
impl AstVisitor for NodeMetaIdentifier {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                NodeMetaIdentifier::MetaName(name) => name.node.visit(emitter),
                NodeMetaIdentifier::MetaNameInNamespace(name, ns) => match name.node.visit(emitter)
                {
                    Err(msg) => Err(msg),
                    _ => ns.node.visit(emitter),
                },
                NodeMetaIdentifier::MetaNameInHexspace(_, name) => name.node.visit(emitter),
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                NodeVariableIdentifier::VariableInNamespace(type_name_identifier, _) => {
                    type_name_identifier.node.visit(emitter)
                }
...
impl AstVisitor for NodeBuiltinArguments {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            ...
            self.arguments
                .iter()
                .map(|argument| argument.node.visit(emitter))
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                ...
                NodeTypeArgument::MapTypeArgument(key_node, value_node) => {
                    match key_node.node.visit(emitter) {
                        Ok(TraversalResult::Continue) => value_node.node.visit(emitter),
                        Err(msg) => Err(msg),
                        _ => Ok(TraversalResult::Continue),
                    }
                }
    ...
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                ...
                NodeScillaType::MapType(key, value) => match key.node.visit(emitter) {
                    Err(msg) => Err(msg),
                    _ => value.node.visit(emitter),
                },
...
impl AstVisitor for NodeTypeMapEntry {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self.key.node.visit(emitter) {
                Err(msg) => Err(msg),
                _ => self.value.node.visit(emitter),
            }
        } else {
            ret
        }?;
...
impl AstVisitor for NodeAddressType {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            let mut ret = self.identifier.node.visit(emitter);
            for field in &self.address_fields {
                match ret {
                    Err(msg) => return Err(msg),
                    _ => ret = field.node.visit(emitter),
                }
            }
            ret
        } else {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                NodeFullExpression::LocalVariableDeclaration {
                    ...
                    ..
                } => match expression.node.visit(emitter) {
                    Err(msg) => Err(msg),
                    _ => containing_expression.node.visit(emitter),
                },
                NodeFullExpression::FunctionDeclaration { expression, .. } => {
                    expression.node.visit(emitter)
                }
                NodeFullExpression::FunctionCall {
                    function_name,
                    argument_list,
                    ..
                } => match function_name.node.visit(emitter) {
                    Err(msg) => Err(msg),
                    _ => {
                ...
                NodeFullExpression::ExpressionAtomic(atom_expr) => atom_expr.node.visit(emitter),
                NodeFullExpression::ExpressionBuiltin { xs, .. } => xs.node.visit(emitter),
                ...
                NodeFullExpression::Match {
                    ...
                    clauses,
                    ..
                } => match match_expression.node.visit(emitter) {
                    Err(msg) => Err(msg),
                    _ => {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                NodeMessageEntry::MessageLiteral(var_identifier, value_literal) => {
                    match var_identifier.node.visit(emitter) {
                        Err(msg) => Err(msg),
                        _ => value_literal.node.visit(emitter),
                    }
                }
                NodeMessageEntry::MessageVariable(var_identifier1, var_identifier2) => {
                    match var_identifier1.node.visit(emitter) {
                        Err(msg) => Err(msg),
                        _ => var_identifier2.node.visit(emitter),
                    }
                }
            }
        } else {
...
impl AstVisitor for NodePatternMatchExpressionClause {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let expression_ret = if pattern_ret == Ok(TraversalResult::Continue) {
            self.expression.node.visit(emitter)
        } else {
            pattern_ret
        }?;
...
impl AstVisitor for NodeAtomicExpression {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                NodeAtomicExpression::AtomicSid(sid) => sid.node.visit(emitter),
                NodeAtomicExpression::AtomicLit(lit) => lit.node.visit(emitter),
            }
        } else {
            ret
        }?;
...
impl AstVisitor for NodeContractTypeArguments {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            self.type_arguments
                .iter()
                .map(|child| child.node.visit(emitter))
...
impl AstVisitor for NodeValueLiteral {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                NodeValueLiteral::LiteralInt(type_name, _) => type_name.node.visit(emitter),
                NodeValueLiteral::LiteralEmptyMap(type_map_key, type_map_value) => {
                    match type_map_key.node.visit(emitter) {
                        Err(msg) => Err(msg),
                        _ => type_map_value.node.visit(emitter),
                    }
                }
                _ => Ok(TraversalResult::Continue),
            }
...
impl AstVisitor for NodeMapAccess {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            self.identifier_name.node.visit(emitter)
        } else {
            ret
        }?;
        match children_ret {
    ...
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                ...
                NodeArgumentPattern::PatternArgument(pattern) => pattern.node.visit(emitter),
            }
        } else {
            ret
        }?;
...
impl AstVisitor for NodePatternMatchClause {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self.pattern_expression.node.visit(emitter) {
                Err(msg) => Err(msg),
                _ => match &self.statement_block {
                    Some(stmt_block) => stmt_block.node.visit(emitter),
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                ...
                NodeStatement::Iterate {
                    ...
                    component_id.node.visit(emitter)
                }
                _ => Ok(TraversalResult::Continue),
            }
        } else {
            ...
            match self {
                ...
                NodeRemoteFetchStatement::ReadStateMutableMapAccess(_, _, _, accesses) => {
                    for access in accesses {
                        if let Err(msg) = access.node.visit(emitter) {
                            return Err(msg);
                        }
                    }
                    Ok(TraversalResult::Continue)
    ...
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            for param in &self.parameters {
                match param.node.visit(emitter) {
                    Err(msg) => return Err(msg),
                    _ => {}
                }
            }
...
impl AstVisitor for NodeParameterPair {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            self.identifier_with_type.node.visit(emitter)
        } else {
            ret
        };
        match children_ret? {
    ...
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            if let Some(statement_block) = &self.statement_block {
                statement_block.node.visit(emitter)
            } else {
                Ok(TraversalResult::Continue)
            }
        } else {
...
impl AstVisitor for NodeTypedIdentifier {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            self.annotation.node.visit(emitter)
        } else {
            ret
        };
    ...
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            for definition in &self.definitions {
                match definition.node.visit(emitter) {
                    Err(msg) => return Err(msg),
                    _ => continue,
                };
            }
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                ...
                NodeLibrarySingleDefinition::TypeDefinition(name, option_clause) => {
                    match name.node.visit(emitter) {
                        Err(msg) => Err(msg),
                        _ => match option_clause {
                            Some(clauses) => {
                                for clause in clauses {
                                    if let Err(msg) = clause.node.visit(emitter) {
                                        return Err(msg);
                                    }
                                }
                                Ok(TraversalResult::Continue)
...
impl AstVisitor for NodeContractField {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self.typed_identifier.node.visit(emitter) {
                Err(msg) => Err(msg),
                _ => self.right_hand_side.node.visit(emitter),
            }
        } else {
            ret
        };
...
impl AstVisitor for NodeProcedureDefinition {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self.name.node.visit(emitter) {
                Err(msg) => Err(msg),
                _ => match self.parameters.node.visit(emitter) {
                    Err(msg) => Err(msg),
...
impl AstVisitor for NodeTypeAlternativeClause {
    fn visit(&self, emitter: &mut dyn AstConverting) -> Result<TraversalResult, String> {
        ...
        let children_ret = match self {
            NodeTypeAlternativeClause::ClauseType(type_name) => type_name.node.visit(emitter),
        ...
        let children_ret = if ret == Ok(TraversalResult::Continue) {
            match self {
                ...
                NodeTypeMapValueArguments::MapKeyValueType(key_type, value_type) => {
                    match key_type.node.visit(emitter) {
                        Err(msg) => Err(msg),
                        _ => value_type.node.visit(emitter),
                    }
                }
            }
        } else {

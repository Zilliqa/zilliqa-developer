...
1. Launch script downloads the latest persistence from AWS S3 [incremental DB](../data-layer/core-incremental-db.md) using `download_incr_db.py`
...
1. Send request to all upper seeds to remove the node's IP address from their [relaxed blacklist](../network-layer/core-blacklist.md)

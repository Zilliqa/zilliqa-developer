...
3. Zilliqa process retrieves Persistence Storage.
...
3. Fetch Latest DSBlocks and Latest TxBlocks from random upper seed.
...
5. If not vacuous epoch,
    a) Fetch the latest Sharding Structure from random upper seed and identify if already part of any shard.
...
6. If vacuous epoch, Fetch Latest DS Committee Info and send request to random upper seed to let him know when to start pow.
7. On receiving notification message from seed, start Init Mining and submit PoW.
8. If received DSBlock within timeout and finds himself in sharding information, change `syncType to NO_SYNC`. Stop blocking messages. **The normal node now successfully joined the network as Shard Node** .
9. If timedout,
    a) Try fetching latest DSBlock from random seed. If successfully got new DSBlock means lost pow this time.
...
3. Retrieves Persistence Storage.
4. It regenerate the current state using base state and state-deltas already fetched from incremental db.
5. syncType is not No_SYNC. so it block some messages that will be received as a healthy normal node.
6. It starts synchronization with `Node::StartSynchronization`.
7. Done.
...
2. Download Persistence from S3 incremental DB.
3. It retrieves Persistence Storage.
...
2. Fetch Latest DSBlocks and Latest TxBlocks from random upper seed.
...
5. Fetch latest DSInfo and Set `syncType = NO_SYNC` and seed is ready.
...
2. It retrieves Persistence Storage.
3. It starts syncronization with `Lookup::InitSync`.
4. Done.

### `Lookup::StartSynchronization`

1. It fetches latest TxBlock and DSBlock from random upper seed.
...
4. Fetch latest DSInfo and Set `syncType = NO_SYNC` and seed/lookup is ready again.

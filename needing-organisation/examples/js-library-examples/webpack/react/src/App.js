...
function App() {
  ...
  const sendPayment = async () => {
    ...
    const isGasSufficient = myGasPrice.gte(
      new zilWindow.BN(minGasPrice.result)
  ...
  useEffect(() => {
    async function initialSetup() {
      ...
      setAddress(address);
      setBalance(
        zilWindow.units.fromQa(
          new zilWindow.BN(balance.result.balance),
          zilWindow.units.Units.Zil
        )
  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <p>
          <small>Balance: {balance} ZIL</small>
        </p>
        {txnId ? (
          <>

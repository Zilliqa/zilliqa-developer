...
const {
  deployFixedPriceContract,
} = require("../scripts/marketplace/deployFixedPriceContract.js");
...
describe("Native ZIL", () => {
  beforeEach(async () => {
    ...
    const newSellOrder = Object.keys(sellOrders).filter((order) =>
      order.includes(1)
    );
    ...
    expect(tx.receipt.exceptions).toEqual([
      {
        line: 1,
        message:
          'Exception thrown: (Message [(_exception : (String "Error")) ; (code : (Int32 -19))])',
      },
    ...
    expect(tx.receipt.exceptions).toEqual([
      {
        line: 1,
        message:
          'Exception thrown: (Message [(_exception : (String "Error")) ; (code : (Int32 -9))])',
      },
    ...
    expect(tx.receipt.exceptions).toEqual([
      {
        line: 1,
        message:
          'Exception thrown: (Message [(_exception : (String "Error")) ; (code : (Int32 -13))])',
      },
  ...
  test("SetOrder: Buyer creates buy order for token #1", async () => {
    ...
    expect(parseInt(fixedPriceEndingBalance)).toBe(
      parseInt(fixedPriceStartingBalance) + parseInt(salePrice)
    );
          ...
          [1]: {
            [zero_address]: {
              [10000]: scillaJSONVal(
                ...
                [
                  accounts.nftBuyer.address,
                  contractState.buy_orders[nftTokenAddress.toLowerCase()][1][
                    zero_address.toLowerCase()
                  ][10000].arguments[1],
                ]
              ),
    ...
    expect(tx.receipt.exceptions).toEqual([
      {
        line: 1,
        message:
          'Exception thrown: (Message [(_exception : (String "Error")) ; (code : (Int32 -6))])',
      },
    ...
    expect(tx.receipt.exceptions).toEqual([
      {
        line: 1,
        message:
          'Exception thrown: (Message [(_exception : (String "Error")) ; (code : (Int32 -17))])',
      },
  ...
  test("FulfillOrder: Buyer fullfills sell order (IS collection item)", async () => {
    const fixedPriceContract = await zilliqa.contracts.at(fixedPriceAddress);
    const collectionContract = await zilliqa.contracts.at(
      collectionContractAddress
    );
    ...
    const eventCommissionFeePaid = tx.receipt.event_logs.filter(
      (e) => e._eventname === "CommissionFeePaid"
    )[0];
    ...
    expect(tx.receipt.exceptions).toEqual([
      {
        line: 1,
        message:
          'Exception thrown: (Message [(_exception : (String "Error")) ; (code : (Int32 -12))])',
      },
      { line: 1, message: "Raised from RequireNotPaused" },
  ...
  test("CancelOrder: Buyer cancels buy order", async () => {
    ...
    expect(txEvent.params[0].value).toEqual(
...
describe("Wrapped ZIL", () => {
  beforeEach(async () => {
    // First we succesfully create a sell order
    const fixedPriceContract = await zilliqa.contracts.at(fixedPriceAddress);
    const paymentTokenContract = await zilliqa.contracts.at(
      paymentTokenAddress
    );
    const notAllowedpaymentTokenContract = await zilliqa.contracts.at(
    ...
    expect(tx.receipt.exceptions).toEqual([
      {
        line: 1,
        message:
          'Exception thrown: (Message [(_exception : (String "Error")) ; (code : (Int32 -15))])',
      },
      { line: 1, message: "Raised from RequireNotExpired" },
      { line: 1, message: "Raised from RequireAllowedUser" },
      { line: 1, message: "Raised from RequireNotPaused" },
  ...
  test("SetOrder: Seller creates sell order for token #1", async () => {
    ...
    console.log(
      "contractStateStart",
      JSON.stringify(contractStateStart.sell_orders)
    );
    let oldExpiryBlock =
      contractStateStart.sell_orders[nftTokenAddress.toLowerCase()][1][
        paymentTokenAddress.toLowerCase()
      ]["10000"].arguments[1];
    ...
    const contractState = await fixedPriceContract.getState();
    const sellOrders = contractState.sell_orders;
    const newBuyOrder = Object.keys(sellOrders).filter((order) =>
      order.includes(2)
    );
  ...
  test("SetOrder: Buyer creates buy order for token #1", async () => {
    ...
    const fixedPriceStartingBalance = await getZRC2State(
      fixedPriceAddress,
      paymentTokenAddress
    );
    console.log(fixedPriceStartingBalance, "fixedPriceStartingBalance");

    const buyerStartBalance = await getZRC2State(
      accounts.nftBuyer.address,
      paymentTokenAddress
    );
    ...
    let oldExpiryBlock =
      contractStateStart.buy_orders[nftTokenAddress.toLowerCase()][1][
        paymentTokenAddress.toLowerCase()
      ]["10000"].arguments[1];
    console.log("oldExpiryBlock", oldExpiryBlock);
    ...
    const fixedPriceEndingBalance = await getZRC2State(
      fixedPriceAddress,
      paymentTokenAddress
    );
    console.log(fixedPriceEndingBalance, "fixedPriceEndingBalance");

    const buyerEndBalance = await getZRC2State(
      accounts.nftBuyer.address,
      paymentTokenAddress
    );
    ...
    expect(parseInt(buyerEndBalance)).toBe(
      parseInt(buyerStartBalance) - parseInt(salePrice)
    );
...
describe("Native ZIL & ZRC2", () => {
  beforeEach(async () => {
    ...
    const paymentTokenContract1 = await zilliqa.contracts.at(
      paymentTokenAddress1
    );
    const paymentTokenContract2 = await zilliqa.contracts.at(
      paymentTokenAddress2
    );
  ...
  test("SetOrder: Seller creates multiple sell order for token #1 with Native ZIL & ZRC2", async () => {
    ...
    const txZilEvent = txZil.receipt.event_logs.filter(
      (e) => e._eventname === "SetOrder"
    )[0];
    ...
    const txwZil = await createOrder(
      tokenId,
      "20000",
      side,
      expiryBlock,
    ...
    const txwZilEvent = txwZil.receipt.event_logs.filter(
      (e) => e._eventname === "SetOrder"
    )[0];
    ...
    const txSGDEvent = txSGD.receipt.event_logs.filter(
      (e) => e._eventname === "SetOrder"
    )[0];
    ...
    const txIDREvent = txIDR.receipt.event_logs.filter(
      (e) => e._eventname === "SetOrder"
    )[0];

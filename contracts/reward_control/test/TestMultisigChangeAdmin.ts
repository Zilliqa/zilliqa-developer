...
describe(utils.TestGeneral(0, "TestMultiSigChangeAdmin"), function () {
  ...
  before(async function () {
    ...
    let non_owner = utils.getZilliqaAddressForAccountByHardhatWallet(WALLET_INDEX_3);
    if (DEBUG) { console.log(`Non-owner address ${non_owner}`); }
    if ((await hre.zilliqa.getBalanceForAddress(non_owner))[0].isZero()) {
      throw Error(`Account {non_owner} is not funded`);
    }
    if (DEBUG) { console.log(`All owners and non-owner have non-zero funds: good!`); }
  ...
  it("Multi sig wallet can change admin to a new multisig wallet", async function () {
    ...
    let num_of_required_signatures = 2
    let scillaMultiSigRewardsParamContractNew = await hre.deployScillaContract("MultiSigWalletRewardsParam", owner_list, num_of_required_signatures);
    ...
    if (DEBUG) { console.log("[1] Multisig txn id is: ", multisig_txn_id) }

    if (DEBUG) { console.log("Get balance for idx 1"); }
    ...
    if (DEBUG) { console.log("[2] Multisig txn id is: ", multisig_txn_id_1) }

    if (DEBUG) { console.log("Signing .. "); } 
    ...
    expect(await scillaRewardsParamsContract.contractadmin()).to.be.eq(multiSigAddressNew.toLowerCase());
  });
});

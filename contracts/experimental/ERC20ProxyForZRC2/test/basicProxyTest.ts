...
describe("basicTest", function () {
  ...
  it("0001 Should report parameters correctly", async function () {
    ...
    expect((await erc20Proxy.zrc2_proxy()).toLowerCase()).to.equal(zrc2Contract.address.toLowerCase());
  });

  it("0002 Should deal with transfers correctly", async function () {
    const AMT=540;
    expect(await erc20Proxy.balanceOf(zrc2OwnerEVM.address)).to.equal(ZRC2_SUPPLY);
    expect(await erc20Proxy.balanceOf(tokenHolder.address)).to.equal(0);
    await (await erc20Proxy.connect(zrc2OwnerEVM).transfer(tokenHolder, AMT)).wait();
    expect(await erc20Proxy.balanceOf(tokenHolder.address)).to.equal(AMT);
    expect(await erc20Proxy.balanceOf(zrc2OwnerEVM.address)).to.equal(ZRC2_SUPPLY - AMT);
    await (await erc20Proxy.connect(tokenHolder).transfer(zrc2OwnerEVM, AMT)).wait();
   });
  ...
  it("0004 should fail transferFrom if the user doesn't have allowance", async function () {
    expect(await erc20Proxy.allowance(zrc2OwnerEVM.address, tokenHolder.address)).to.equal(0);
    await expect(erc20Proxy.connect(tokenHolder).transferFrom(zrc2OwnerEVM, proxyDeployer, 1)).to.be.reverted;
  });
  ...
  it("0005 should succeed transferFrom if there is enough allowance", async function () {
    ...
    await(await erc20Proxy.connect(zrc2OwnerEVM).approve(proxyDeployer, AMT)).wait();
    expect(await erc20Proxy.allowance(zrc2OwnerEVM.address, proxyDeployer.address)).to.equal(AMT);
    await (await erc20Proxy.connect(proxyDeployer).transferFrom(zrc2OwnerEVM, tokenHolder, AMT)).wait();
    expect(await erc20Proxy.balanceOf(zrc2OwnerEVM.address)).to.equal(ZRC2_SUPPLY - AMT);
    expect(await erc20Proxy.balanceOf(tokenHolder.address)).to.equal(AMT);
    expect(await erc20Proxy.balanceOf(proxyDeployer.address)).to.equal(0);
  });

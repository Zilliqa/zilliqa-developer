...
describe("burnTest", function () {
  ...
  before (async function() {
    ...
    if (!process.env.CACHED) {
      zrc2Contract = await hre.deployScillaContract("FungibleTokenBurnable",
      ...
      const erc20Factory = (await ethers.getContractFactory("ZRC2ERC20ProxyBurnable")).connect(proxyDeployer);
      erc20Proxy = await erc20Factory.deploy(zrc2Contract.address.toLowerCase());
      await erc20Proxy.waitForDeployment();
    } else {
      zrc2Contract = await hre.interactWithScillaContract("0x5aDEf0BbBEbD54831E2300Aa58f58d18f5E8E66F");
      erc20Proxy = (await ethers.getContractFactory("ZRC2ERC20ProxyBurnable")).connect(proxyDeployer).attach("0x7C670086208545f8D49edeef2bc351dD0667B932");
    }
  ...
  it("B0001 Should be able to burn tokens that you own", async function () {
    ...
    expect(await erc20Proxy.balanceOf(zrc2OwnerEVM.address)).to.equal(balance - BigInt(AMT));
  ...
  it("B0002 Should be able to burn tokens from an allowance", async function () {
    ...
    await(await erc20Proxy.connect(zrc2OwnerEVM).approve(tokenHolder.address, AMT)).wait();
    await(await erc20Proxy.connect(tokenHolder).burnFrom(zrc2OwnerEVM.address, AMT)).wait();
  ...
  it("B0003 Should not be able to burn tokens you don't own", async function () {
    ...
    const allowance = await erc20Proxy.allowance(zrc2OwnerEVM.address, tokenHolder.address);
    expect(allowance).to.equal(BigInt(0));
    await expect(erc20Proxy.connect(tokenHolder).burnFrom(zrc2OwnerEVM.address, AMT)).to.be.reverted;
  });
